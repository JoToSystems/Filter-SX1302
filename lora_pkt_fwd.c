...
void thread_up(void) {
    ...
    *(uint32_t *)(buff_up + 8) = net_mac_l;
    ...
    while (!exit_sig && !quit_sig) {
		...
		printf("TEST POINT: 1  num packets %u  send report %u \n", nb_pkt, send_report);
        ...
        for (i = 0; i < nb_pkt; ++i) {
			...
			printf("TEST POINT: 3 \n");
            printf( "\n Test Point 3a: Received pkt from mote: %08X (fcnt=%u)\n", mote_addr, mote_fcnt );
			...
			if (dev_addr == dev_addr_filter_1) {
				printf("FILTER INFO: Uplink Packet dropped by Filter 1 \n");
				drop_packet = 1;
			}

			else if (dev_addr == dev_addr_filter_2) {
				printf("FILTER INFO: Uplink Packet dropped by Filter 2 \n");
				drop_packet = 1;
			}

			else if (dev_addr == dev_addr_filter_3) {
				printf("FILTER INFO: Uplink Packet dropped by Filter 3 \n");
				drop_packet = 1;
			}

			else if (dev_addr == dev_addr_filter_4) {
				printf("FILTER INFO: Uplink Packet dropped by Filter 4 \n");
				drop_packet = 1;
			}

			else if (dev_addr == dev_addr_filter_5) {
				printf("FILTER INFO: Uplink Packet dropped by Filter 5 \n");
				drop_packet = 1;
			}

			else if (dev_addr == dev_addr_filter_6) {
				printf("FILTER INFO: Uplink Packet dropped by Filter 6 \n");
				drop_packet = 1;
			}

			else if (dev_addr == dev_addr_filter_7) {
				printf("FILTER INFO: Uplink Packet dropped by Filter 7 \n");
				drop_packet = 1;
			}

			else if (dev_addr == dev_addr_filter_8) {
				printf("FILTER INFO: Uplink Packet dropped by Filter 8 \n");
				drop_packet = 1;
			}

			else if (dev_addr == dev_addr_filter_9) {
				printf("FILTER INFO: Uplink Packet dropped by Filter 9 \n");
				drop_packet = 1;
			}

			else if (dev_addr == dev_addr_filter_10) {
				printf("FILTER INFO: Uplink Packet dropped by Filter 10 \n");
				drop_packet = 1;
			}
			
			else {
				printf("FILTER INFO: No Filter Applied \n");
			}
			...
			if (drop_packet == 1) {
				printf("TEST POINT: Drop Packet = True \n");
			}
        ...
        while ((int)difftimespec(recv_time, send_time) < keepalive_time) {
            ...
            if (val != NULL) {
				...
				if (txpkt.rf_power > 27) {
					txpkt.rf_power = 27;
					printf("INFO: Tx Power Reduced to 27dBm \n");
				}
            }
